...
#   --env-file=PATH   installer env file      [default /etc/appximo/appximo.env]
#   --service=NAME    engine unit             [default appximo]
#   --help
set -euo pipefail
...
write_pg_conf() {
	cat > "$TUNE_FILE" <<EOF
# Appximo production tuning — generated by scripts/verify-production/tune.sh
...
detect_pg_conf
TUNE_FILE="$PG_CONF_D/99-appximo-tuning.conf"
