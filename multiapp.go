...
var fleetMappedEnv = map[string]bool{
	...
	"APPXIMO_AUTH_LOGIN_ATTEMPTS_PER_MINUTE": true, "APPXIMO_AUTH_LOGIN_BURST": true,
...
func buildFleetApp(ctx context.Context, mf *fleet.Manifest, spec *fleet.AppSpec, listenPort int, version string, debugTracesHTML []byte) (*App, error) {
	...
	cfg := Config{
		...
		// ENG-47: per-app login limiter. envInt keeps an unparsable value at 0
		// (= default) here; the single-engine path refuses to boot on one.
		AuthLoginAttemptsPerMinute: envInt(env["APPXIMO_AUTH_LOGIN_ATTEMPTS_PER_MINUTE"]),
		AuthLoginBurst:             envInt(env["APPXIMO_AUTH_LOGIN_BURST"]),
