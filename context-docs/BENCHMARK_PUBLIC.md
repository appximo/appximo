# Appximo vs NestJS — Public Comparative Benchmark (S46)

> **Re-certification status (CERTIFY-S1, 2026-08-01).** The **Appximo** figures in
...
**Date:** 2026-06-10 · **Appximo version:** HEAD `bb6cc9f` (S44 declarative validation + S45 SSE subscriptions, the launch build) · **Status:** every claim below is backed by the raw run exports in [`benchmarks/data/s46-pub-runs.csv`](../benchmarks/data/s46-pub-runs.csv).
...
|---|---|---|
| Appximo @ **2000 RPS** sustained | p50 **1.58 ms** (CI95 [1.52, 1.62]), p95 63.0 ms (CI95 [58.7, 68.3])\*, **0 errors** | 10 schedule-clean runs × 30 s |
| Appximo server-side @ 2000 RPS | **99.98% of 114,771 requests < 5 ms, mean 0.079 ms** | Prometheus histogram delta across a dedicated 2-run window |
| NestJS @ **500 RPS** (its highest sustained level) | p50 **7.40 ms** (CI95 [6.42, 9.92]), p95 38.7 ms | 10 schedule-clean runs × 30 s |
| Head-to-head @ 500 RPS | Appximo p50 **1.53 ms** vs NestJS **7.40 ms** → **~4.8× faster**; pooled-median delta CI95 **[+5.54, +5.60] ms**, Mann-Whitney **U=0, p≈0** | 10 vs 10 runs, ABBA windows |
| Head-to-head @ 500 RPS, **response cache disabled** | Appximo p50 **2.76 ms** (CI95 [2.65, 2.86]) — **still ~2.7× faster** than NestJS; delta CI95 **[+4.36, +4.42] ms**, p≈0 | 10 clean runs, every request verified to hit Postgres (§4.4) |
| Saturation point | NestJS **collapses between 500 and 750 RPS** (p50 238–420 ms, p99 up to 27.6 s at 750). Appximo **did not saturate** at any level our loader can reliably drive (≤2000 RPS) | Ladder, §3 |
| Run-to-run stability | between-run CV of p50: Appximo **2.9%**, NestJS **22.8%** | compare-groups |
...
The loader is **never** co-located with the SUT (a loader competing with the SUT for CPU corrupts both sides of the measurement). The ~1.2 ms RTT **floor** is included in every latency number for **both** stacks — which is why Appximo's external p50 (1.53 ms) sits just above it: under sustained keep-alive traffic most round trips ride near the floor, while one-shot pings sample the loader's jitter.

### 2.2 Software versions

| Component | Version |
|---|---|
| Appximo | HEAD `bb6cc9f`, Go **1.25.11**, `GOMAXPROCS=2 GOGC=200 GOMEMLIMIT=1GiB`, rate limiter 3000 RPS/300 burst per tenant (production env) |
...
- Appximo: schema-per-tenant (`tenant_1..10`), 100 k `guides` rows per tenant, 25 k per status.
...
Both stacks return the 20 newest `pending` rows for tenant 10. (NestJS hardcodes `ORDER BY created_at DESC` and ignores the sort params; Appximo honors them. Semantics verified identical before measuring.)

### 2.4 What each stack does per request — declared asymmetries

| Layer | Appximo | NestJS baseline |
|---|---|---|
...
4. The two stacks **never** receive load simultaneously; headline groups ran in **ABBA window order** (A=Appximo ×5, B=NestJS ×5, B ×5, A ×5) to neutralize slow host drift.
...
| RPS | Appximo p50 | p95 | p99 | err | NestJS p50 | p95 | p99 | err |
|---|---|---|---|---|---|---|---|---|
...
**Saturation points:** NestJS sustains **500 RPS** and collapses between 500 and 750. Appximo sustained every level up to **2000 RPS** (p95 60 ms external, 0 errors) without meeting the saturation criterion; 2500 RPS exceeds what our loader can drive reliably, so Appximo's true ceiling on this hardware is **not reached** in this report.

The Appximo external p95 growth at 1500–2000 (16 → 60 ms while p50 stays ~1.4 ms) is dominated by loader/network artifacts, not the engine — see §4.3.
...
- **Mann-Whitney U = 0, p ≈ 0** — the pooled distributions are completely separated; every Appximo latency rank sits below NestJS's.
- **Median difference CI95 = [+5.54, +5.60] ms** (bootstrap, seed 42) — NestJS is ~4.8× slower at the median.
- Pooled p95 delta: **+1182%**.
- Practical-significance gate (≥ max(0.5 ms, 3% of median)) passed by an order of magnitude: verdict **significant regression** for NestJS vs Appximo.

NestJS's 22.8% between-run CV is itself a finding: at 500 RPS it operates near its capacity knee, where small host perturbations produce large latency swings. Its single best run of the whole session (p50 3.91 ms, fully warm) was still **2.6×** Appximo's median.

### 4.2 Appximo flagship: 2000 RPS, 10 clean runs
...
During a dedicated 2-run window at 2000 RPS we diffed the engine's own Prometheus latency histogram (`appximo_request_duration_seconds`, tenant 10, `/api/guides`):

> **114,771 requests: 99.983% completed inside the engine in < 5 ms; mean 0.079 ms.**

The 60 ms external p95 is therefore queueing in the network path and the 1-vCPU loader, not engine latency. (The 0.079 ms mean also shows the response cache doing its job — repeated identical GETs are served from memory; see §2.4.) NestJS exposes no equivalent histogram, so this cross-check is Appximo-only — another declared asymmetry.

---

### 4.4 Appximo with the response cache disabled

The biggest declared asymmetry (§2.4) is the built-in response cache. To isolate it, we re-ran the full 500 RPS headline protocol (10 schedule-clean runs of 30 s, same validity rule, label `pub-appximo-500-nocache`) sending **`Cache-Control: no-cache`** on every request — which the engine honors as a complete cache bypass.
...
| @ 500 RPS, 10 clean runs | Appximo (cache ON) | **Appximo (cache OFF)** | NestJS |
|---|---|---|---|
...
2. **Network between droplets** adds the ~1.2 ms RTT floor to every measurement, for both stacks (see §2.1 on why the ping *average* reads higher). Appximo's external p50 (~1.5 ms) is essentially that network floor.
...
# Appximo (production config):
JWT_SECRET=… ADMIN_KEY=… docker compose --profile appximo up -d   # or the bare binary, see Dockerfile
...
BENCH_TOKEN=$(appximo token --tenant bench10 --secret "$JWT_SECRET" --role admin) \
  bash benchmark-lab/run-pub-bench.sh appximo http://SUT:8080
...
curl -X POST localhost:3099/api/bench/compare-groups \
  -d '{"label_a":"pub-appximo-500","label_b":"pub-nestjs-500"}'
