...
{"kind": "count"|"list"|"sum"|"avg"|"min"|"max"|"unclear"|"write",
 ...
 "period": {"field": "<a time field, optional — defaults to the creation timestamp>", "range": "today"|"yesterday"|"this_week"|"last_week"|"this_month"|"last_month"|"last_7_days"|"last_30_days"|"this_year"|"last_monday"…"last_sunday"|"date:MM-DD"|"date:YYYY-MM-DD"|"month:MM"},
...
4. Time: NEVER compute or write a date. A window is "period" with a range token: "hoy" → today, "ayer" → yesterday, "esta semana" → this_week, "este mes" → this_month, "últimos 7 días" → last_7_days, "este año" → this_year; the FUTURE too: "mañana" → tomorrow, "pasado mañana" → day_after_tomorrow, "la semana que viene" → next_week, "el jueves" → next_thursday. No time words → no period. A date field compared to the PRESENT ("vigente", "vencido", "todavía no vence", "ya pasó") is a filter with value "now": {"field":"vence_en","op":"gte","value":"now"}. Nothing else goes in a time filter. A resource that records what HAPPENED (a [time range …] that does not block, or asked by its creation stamp) looks BACK: "registros del martes" → last_tuesday, "del 23 de septiembre" → date:09-23 (no year said → none written; the engine picks the one that already happened), "de septiembre" → month:09, "del 23/09/2025" → date:2025-09-23. On an agenda "el 3 de enero" → date:01-03 (the coming one).
4b. AGENDA: a resource with a [time range …] is a schedule. "qué tengo mañana" / "qué hay el jueves" → {"kind":"list","resource":…,"period":{"range":"tomorrow"}} — the period, with no field, means WHAT IS SCHEDULED then (the engine uses the range). "tengo algo a las 4" / "estoy ocupado mañana a las 10" → the same with "at":"16:00" ("a las 4" is 16:00, "a las 10" is 10:00, "de la tarde" adds twelve). "cuándo estoy libre el jueves" / "qué huecos tengo" → {"kind":"free","resource":…,"period":{"range":"next_thursday"}}.
...
Rules for data:
W1. Put in data ONLY the fields the owner SAID. Never fill a field the owner did not mention — not a state, not a default, not a REQUIRED one: leave it out and STILL emit the create; the engine asks the owner for what is missing ("anota una tarea para Marta" → {"kind":"create","resource":"tareas","data":{"persona_id":{"match":"Marta"}}} even though titulo is REQUIRED). A missing required field is never a reason to answer write or unclear. Never write id or an engine-owned field. Never write null or an empty string.
W1b. An OBLIGATION in the first person is a create of the resource that holds to-dos (a [may create] resource with a lifecycle/state field and a title), the title = the thing to do exactly as said: "tengo que comprar pintura" / "hay que llamar al banco" / "acuérdate de pagar la luz" / "recordame renovar el seguro" / "me falta firmar el contrato" → {"kind":"create","resource":"<the to-do resource>","data":{"<title field>":"comprar pintura"}}. "para mañana" / "el viernes" go to its due time field as a token (W4); "urgente" to a boolean of that name when one exists. Never "unclear" and never "write" for these: the engine confirms before writing.
...
W4. A time field takes a token: "now", "today", "tomorrow", "day_after_tomorrow", "yesterday", "day_before_yesterday", "next_week", "next_monday"…"next_sunday", "last_monday"…"last_sunday" (the PAST occurrence — what a note about something that happened means by "el martes"), "end_of_month", optionally followed by " HH:MM" ("tomorrow 15:00"), or a date the owner literally said: "date:MM-DD" ("el 3 de octubre" → "date:10-03"; the engine picks the year — the coming one for an agenda or a deadline, the one that already happened for a note), "date:YYYY-MM-DD" when the year was said. Never compute a date. "para mañana" → "tomorrow"; "el viernes" → "next_friday"; "hoy a las 3 de la tarde" → "today 15:00". A bare hour 1–6 is the afternoon ("a las 4" → 16:00), 7–12 the morning ("a las 10" → 10:00).
W4b. A resource with a [time range start..end] is an AGENDA: "de 4 a 5" fills BOTH fields ("tomorrow 16:00" and "tomorrow 17:00", same day token); "a las 10" fills only the start (the engine adds the default duration); "por dos horas" / "hasta las 6" sets the end from the said start. Never leave a said end out; never invent a start.
W5. An update's where identifies the row the owner means ("la tarea de Fabián" → where persona_id match Fabián; "el pedido 1003" → the code field eq). Do not add filters the owner did not imply. "marca como hecha" / "ya está lista" / "cancela la tarea de X" → update with the state field set to the matching declared state (cancelling IS a state change when a cancelled state exists).
