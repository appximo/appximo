...
func GenerateOpenAPI(s *schema.APISchema) ([]byte, error) {
	spec := openAPISpec{
		OpenAPI: "3.0.3",
		Info: map[string]any{
			"title":       s.Name,
...
func oaListOp(name, title string, res *schema.ResourceSchema) map[string]any {
	params := []any{
		oaQueryParam("page", map[string]any{"type": "integer", "default": 1, "minimum": 1}),
		oaQueryParam("per_page", map[string]any{"type": "integer", "default": 20, "minimum": 1, "maximum": 100}),
		oaQueryParam("sort", oaSortSchema(res)),
		oaQueryParam("order", map[string]any{"type": "string", "enum": []string{"asc", "desc"}, "default": "asc"}),
		oaQueryParam("search", map[string]any{"type": "string", "description": "Full-text search across string fields (ILIKE)"}),
	}
	for _, fname := range sortedFieldKeys(res) {
		params = append(params, oaQueryParam("filter["+fname+"]", oaFieldType(res.Fields[fname])))
	}
	return map[string]any{
		...
		"responses": map[string]any{
			...
			"401": oaRespRef("Unauthorized"),
			"403": oaRespRef("Forbidden"),
		},
	}
}
...
func oaCreateOp(name, title string) map[string]any {
	return map[string]any{
		...
		"responses": map[string]any{
			...
			"400": oaRespRef("BadRequest"),
			"401": oaRespRef("Unauthorized"),
			"403": oaRespRef("Forbidden"),
			"422": oaRespRef("UnprocessableEntity"),
		},
	}
}
...
func buildOAComponents(s *schema.APISchema) map[string]any {
	schemas := map[string]any{
		...
		"PaginationMeta": map[string]any{
			"type":     "object",
			"required": []string{"page", "per_page", "total", "total_pages"},
			"properties": map[string]any{
	...
	return map[string]any{
		...
		"schemas": schemas,
		"responses": map[string]any{
			"BadRequest":          oaErrResp("Bad request — invalid input or UUID format"),
			"Unauthorized":        oaErrResp("Authentication required — include Bearer token"),
			"Forbidden":           oaErrResp("Insufficient permissions for this action"),
			"NotFound":            oaErrResp("Resource not found"),
			"UnprocessableEntity": oaErrResp("before_create hook rejected the request"),
		},
	}
}
...
func oaSortSchema(res *schema.ResourceSchema) map[string]any {
	enum := []string{"id"}
