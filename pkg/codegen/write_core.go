...
//  2. the declarative rules (required, enum, min/max, length, pattern, format)
//     AND the value type check, collected together so one response carries
//     every failing field (the S44 contract — reporting them in two phases
//     makes a form UI mark two fields, then two different ones);
...
func PrepareCreate(res *schema.ResourceSchema, rv *schema.ResourceValidator, body map[string]any) []schema.FieldRuleError {
	if rv != nil {
...
// runs. Defaults are deliberately absent — they are create-only, on both paths,
// matching SQL DEFAULT.
//
