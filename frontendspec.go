...
// production FRONTEND on an Appitools backend — where the frontend lives
...
// `appitools frontend-spec` command prints (embedded here so they never
...
// Paste it into your own agent (Claude Code, Cursor) alongside `appitools
// spec` and `appitools backend-spec` and the agent has the full stack: schema,
