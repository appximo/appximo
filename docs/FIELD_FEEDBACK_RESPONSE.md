...
# Response to the FIFTH field report (a real migration) — MIGRACION-CONFIANZA-S1 + MOTOR-FIELDS-S1, 2026-08-28

**Context:** an external agent migrated a **real system** — Symfony 7.2 /
API Platform, 23 tables, 46,119 tax declarations, 1.2 GB of nested JSON — to
Appximo v0.1.10 in five sessions and delivered a nine-point report. Every
point came from ONE path the engine had never exercised: *receiving a system
that already exists* (atina, VecinGo and the demo apps were all born new).
The raw report is the evaluator's text and is not reproduced here; each
finding is restated in its own words below, with what was verified and what
changed. Three of the nine were closed the same day; six are registered in
[BACKLOG.md §MIG-FRONT](BACKLOG.md#mig-front--the-receive-a-system-that-already-exists-front-six-findings-from-a-real-migration-a-product-decision-pending-miguel)
with the report's evidence, as a product decision — one of them (#5) was
built in the next session. **Three of the report's diagnoses were wrong, and
the corrections are part of the record: the cycle worked in both directions.**

| Finding (report's severity) | Verified | Status |
|---|---|---|
| **1. "No bulk import — and `/api/transaction` does not exist in v0.1.10"** (blocking) | Probed request by request against the PUBLISHED v0.1.9 and v0.1.10 binaries (checksums verified) and `main`: `POST /api/transaction` exists in every version — 2 creates → 200, a read-only role → 403 naming op 0, `GET` → 405, 101 ops → `400 too many operations (max 100)`, 100 ops → 200. It has existed since 2026-06, before the first tag. **The gap was the CONTRACT:** `/openapi.json` — which the engine calls "the authority for existence" — did not list it in any version, so an agent reading the contract concluded, correctly by that contract, that it was absent. | **The diagnosis was wrong; the cause was ours.** The route is now published in the served `/openapi.json` (`x-appximo-transaction`, request/response components, `maxItems: 100`) and backend-spec §2b documents "writing from outside the binary" (measured: 100 creates ≈ 50–70 ms on the dev box; 46k rows ≈ 460 batches ≈ minutes). What stays true: no `COPY`, no file importer — a `COPY`-class door is registered (MIG-FRONT #1), to be built only with a customer's volume behind it. |
| **2. "The engine applies no memory backpressure"** (blocking, "the most expensive") | Postgres on a 957 MiB box shared by five apps was OOM-killed during the load. The evaluator's own correction, recorded: the box had **no swap**; with 2 GB of swap the same load was absorbed. Audit of the engine: no notion of host pressure existed (`GOMEMLIMIT` bounds only its own heap — which was never the problem; the pool is a fixed 10 connections; the limiter counts requests). | **Partly environmental, one engine seam closed.** `APPXIMO_MEMORY_GUARD_MIN_MB`: while `MemAvailable + SwapFree` (never `MemAvailable` alone — `shared_buffers` shows as Cached and is not reclaimable) is under a floor (default max(32 MiB, 2 % of RAM), `0` disables), data-plane WRITES answer a 503 that names the numbers and the knob (+ `Retry-After: 5`); reads keep flowing. **This is degradation, not capacity** — the engine does not make 957 MiB hold five apps and a firehose, and no doc promises it. The installer now warns loudly on a ≤ 2 GiB box with no swap, with the recipe. A host-derived pool size stays registered (MIG-FRONT #2). |
| **3. "`install.sh` keeps the binary and the schema on an existing `--app`"** (high) | Reproduced with REAL installs in an LXD container: the "Update" path the installer's own summary prints (without `--schema`) kept whatever schema was there — with VecinGo's under `/etc/retotr/`, `GET /api/asambleas → 200` on the Reto Tributario domain. The binary half did not reproduce (with `--binary` it was replaced), but the fix covers it regardless. The two minor items: the companion scripts were not installed because of an internal `cd /tmp` resolving a relative `$0` — **not the missing execute bit the report diagnosed** — and the `appximo-cli` symlink is correct for the stock engine but breaks when a consumer binary is installed later without `--cli`. | **CLOSED — the most serious item of the report.** The upgrade criterion is now WRITTEN (secrets/database/data KEPT; binary/unit/Caddy site/companions ALWAYS replaced; the schema replaced with `--schema`, else kept and VERIFIED to be this app's — byte-identical to, or bearing the `name` of, another app's schema on the box → the install STOPS before mutating, naming the other app). At the end the installer verifies installed == asked (binary sha256; `/health` version locally AND through Caddy; schema on disk) and exits 1 on any mismatch even with the service up. Port preflight decides by PID; companions resolve from the script's own directory; the stale symlink is removed and named. Three real paths verified (clean / legitimate upgrade / foreign schema) plus a port collision. |
| **4. "Historical timestamps cannot be imported"** (high for migrations) | `created_at`/`updated_at` declared `auto` reject a client value with 422 — by design (WRITE-ASYMMETRY-S1: the audit trail is the engine's). The evaluator identified the `import` declaration as the legitimate door and did not use it because it changes the schema. | **Documentation.** The door existed and is the declared way to bring history in: `"import": {"roles": ["admin"], "fields": ["id", "created_at"]}` — create-only, role-named, greppable. It is now in backend-spec §2b and the migration checklist. No engine work unless a migration needs update-side history, which `import` refuses on purpose. |
| **5. "Lists return the heavy fields whole"** (medium) | `GET /api/declarations` returned the whole `data` document per row although it paginated: ~940 KB per page of 20, p99 3.8 s — the first screen a client of a migrated system sees. Rebuilt on the dev box (46,119 rows × ~52 KB, TOAST 1.8 GB) and measured with `pg_statio`: the cost is the **detoast on disk**, not the wire — a projection applied in Go would keep every block. | **BUILT (MOTOR-FIELDS-S1, ADR-029).** `?fields=a,b,c` is pushed into the SQL `SELECT` on every row-returning read (list, get, subroute, the root of `?include=`, admin browse, `ctx.Query`), and GraphQL pushes its selection set the same way (it used to select in Go over `SELECT *`). Measured on the rebuilt case: page 961 KB / 53 ms → 3 KB / 1.2 ms; 10 rps p99 2.8 s → 175 ms ([BENCHMARKS §4b](BENCHMARKS.md#4b-field-selection--the-disk-not-the-wire-motor-fields-s1)). `id` always; unknown → 400 naming it and the available set; a field the role hides → omitted. The `/app` sends it on every list. Default omission of heavy fields is NOT built — it is a contract break — and is registered as SCHEMA-8, a declaration. |
| **6. `Ctx` limits for custom routes** (medium) | (a) no response-header setter → no `ETag` from a handler; (b) no in-process event subscription to invalidate a cache; (c) a `Public` route leaves the caller anonymous, so `ctx.Query` is forbidden unless `rbac.public` is declared. | **Registered, not built** (MIG-FRONT #6): three small seams — `Ctx.SetHeader`, an `OnChange(resource, fn)` fed by the post-commit fan-out, and `Route.AsRole` (run a public handler's reads as a NAMED read-only role — the clean answer to "a public catalogue endpoint without opening `rbac.public`"). None blocks a migration. |
| **7. "The engine canonicalizes `json` and nobody said so"** (medium, parity) | Verified with requests on `json` AND `jsonb`: keys re-sorted (recursively; array order kept), floats re-rendered in short form (`0.0100…0859375` → `0.01` and `1.50` → `1.5` — the same float64, no loss), integers past 2^53 truncated (`12345678901234567890` → `…67000` — the one real loss, ENG-50, both directions, both types). Byte identity is not reachable through the API for a value written as a value. | **Documented, with the exact door.** backend-spec §2, SCHEMA_REFERENCE §3 and the ADR-028 addendum carry the table, the parity recipe (canonicalize both sides — Python / Go / `a::jsonb = b::jsonb`) and the one exact door: a JSON-text STRING on a `json` (TEXT) field keeps its numeric text and key order (compacted). "Exact bytes preserved" survives in no doc (swept: 0 live copies). |
| **8. Schema expressivity** (low) | Partial unique indexes, composite primary keys and a per-resource `rate_limit` could not be expressed. | **Decided / documented.** Partial indexes: CLOSED by ADR-022 D2 (Postgres normalizes predicates → diff churn; put them in `Config.BeforeStart` DDL). Composite PK: by design — the implicit `id` + a composite `unique` index expresses the constraint, the FK side is `foreign_keys`. `rate_limit` per resource: a defensible schema key, registered (MIG-FRONT #8). The three are written down as the known DDL translation table. |
| **9. The validator's false positive** (low, "costs trust") | Reproduced with the report's exact shape: `related_user_id → users`, role scoped by `related_user_id = $user_id`. At runtime the condition WORKED (admin sees 6, one accountant 5, another 1, cross read 404) while the validator asserted "matches NO rows, ever" and suggested renaming a column that came from the source DDL. | **CLOSED, with a rule.** The check stays (it caught a real deployment with zero rows) but is now worded as a QUESTION: it names both id spaces, says which case is fine and which is not, says the validator cannot tell them apart ("one request as a scoped role answers it"), puts "keep the column exactly as it is" FIRST and never asks for a rename; the other answer is a declaration (`references: "user_id"`). Written rule (code + AGENTS.md): **the validator never pushes an author to change a name that comes from an existing DDL — parity with the source is a value, not an obstacle.** |

**What the report got right that we did not know:** the installer's silent
schema carry-over (#3) is the most dangerous defect any evaluation has found
in this project — a resource of another application served under a new
domain, with no error anywhere. **What the report got wrong, kept on the
record:** the batch endpoint existed (#1 — our contract hid it), the OOM was
the missing swap (#2 — the evaluator's own correction), and the companion
scripts failed for a `cd`, not an execute bit (#3, minor). Both directions
count: a field report is a measurement, and a measurement is corrected, not
defended.
