...
> | 3 | No "empty server → running app" automation | PROD-PATH-BUILD-S1 + GOLD — `scripts/install.sh`, proven on a real VPS (`api.appximo.com`) |
...
# PROD-PATH-AUDIT-V1 — Camino de producción de Appximo (inventario + gaps)

Auditoría de VERIFICACIÓN pura (cero construcción) del estado real, HOY, de todo
lo que Appximo tiene para producción: Docker, compose, deploy, TLS, dominios,
...
- **Build nativo:** `go build ./cmd/appximo` → **OK** (exit 0). Binario canónico
...
|-------|--------|---------------------|
| `Dockerfile` | 🔴 **NO buildea en HEAD** | Multi-stage `golang:1.25-alpine` → `alpine:3.21`; usuario **no-root** (`appximo`); estático CGO_ENABLED=0; **una imagen dos roles** (engine por defecto, `worker` por keyword — `deploy/docker-entrypoint.sh`); `HEALTHCHECK` contra `/healthz`; hornea `examples/quickstart/schema.json`; crea `/var/lib/appximo/{files,obs}`. **Rompe** por el `go:embed docs/` (hallazgo #1). |
...
| systemd | 🟡 solo en docs | `docs/DEPLOY.md §3.2/3` trae `appximo.service` + `appximo-worker.service` como copy-paste (con `StateDirectory`, `NoNewPrivileges`, `EnvironmentFile`). **No hay `.service` versionado** para el engine (solo `tools/devhub/devhub.service`, que es del devhub). |
...
- **CORS:** ✅ configurable, **off por defecto**, por `APPXIMO_CORS_*` (o `Config`), scope `/api,/auth,/graphql,/openapi`. `app.go:674`.

### 1.4 Frontend estático

- **El binario NO sirve estáticos arbitrarios.** Solo monta SPAs **embebidas** vía `go:embed`: `/editor` (Studio, `pkg/editorui`), `/admin` (panel, `pkg/adminui`), `/docs` (Swagger), `/graphiql`. No hay flag `--static-dir`, ni `APPXIMO_STATIC_DIR`, ni `http.FileServer` sobre un directorio de usuario.
...
| Aspecto | Estado |
|---------|--------|
| `APPXIMO_ENV=development` | Enciende pprof (`:6060`, override `APPXIMO_PPROF_PORT`), introspection GraphQL + GraphiQL. Producción (unset/otro) los apaga (GraphiQL solo con `APPXIMO_GRAPHQL_PLAYGROUND=on`). `app.go:1053,1151`. |
...
| Backups | `POST /admin/backup?tenant=ID` (pg_dump custom+comprimido) — **requiere `postgresql16-client` en la imagen, NO incluido por defecto** (documentado en el Dockerfile). CLI `appximo backup`. **Sin schedule/tooling automático** (manual/cron). `BACKUP_DIR` opcional. |
...
**Runtime/infra (opcionales):** `APPXIMO_ENV`, `APPXIMO_CONTROL_PORT`, `APPXIMO_PPROF_PORT`, `GOMAXPROCS`, `GOMEMLIMIT`, `GOGC`, `DB_MAX_CONNS`, `RATE_LIMIT_RPS`, `RATE_LIMIT_BURST`, `APPXIMO_MAX_TX_OPS`, `APPXIMO_MAX_SSE_PER_TENANT`, `REDIS_URL`, `OBS_DB_PATH`, `BACKUP_DIR`, `SLACK_WEBHOOK_URL`, `APPXIMO_SYNTHETIC`, `APPXIMO_SAFEGO_TIMEOUT`, `APPXIMO_PUBLIC_ROUTE_RPS/BURST`.

**Files:** `APPXIMO_FILES_BACKEND`, `APPXIMO_FILES_DIR`, `APPXIMO_FILES_MAX_BYTES`, `APPXIMO_FILES_TOKEN_TTL`, `APPXIMO_FILES_ALLOWED_EXT`, `APPXIMO_FILES_S3_{BUCKET,ENDPOINT,REGION,ACCESS_KEY,SECRET_KEY,FORCE_PATH_STYLE,PREFIX,SERVE}`.

**Auth/OAuth/MFA:** `APPXIMO_AUTH_SIGNUP_ROLE`, `APPXIMO_AUTH_MIN_PASSWORD`, `APPXIMO_AUTH_REQUIRE_VERIFIED`, `APPXIMO_AUTH_BASE_URL`, `APPXIMO_EMAIL_TOPIC`, `APPXIMO_OAUTH_{GOOGLE,GITHUB,MICROSOFT}_CLIENT_ID/_CLIENT_SECRET`, `APPXIMO_OAUTH_CALLBACK_URL`, `APPXIMO_OAUTH_DEFAULT_ROLE`, `APPXIMO_OAUTH_SUCCESS_REDIRECT`, `APPXIMO_MFA_KEY`, `APPXIMO_MFA_ISSUER`.

**Platform admin / GraphQL / Fleet:** `APPXIMO_PLATFORM_SUPER_ADMIN_ROLE`, `APPXIMO_PLATFORM_MFA_ISSUER`, `APPXIMO_GRAPHQL_PLAYGROUND`, `APPXIMO_FLEET_OPERATOR_KEY`, `APPXIMO_FLEET_ADMIN_EMAIL`, `APPXIMO_FLEET_ADMIN_PASSWORD`.

**Worker (binario separado):** `DATABASE_URL`, `JWT_SECRET`, `APPXIMO_WORKER_MODE`, `APPXIMO_ENGINE_URL`, `APPXIMO_TENANT_DOMAIN`, `APPXIMO_WORKER_ROLE`, `APPXIMO_FILES_DIR`, `SMTP_{HOST,PORT,USER,PASS,FROM}`, `APPXIMO_EMAIL_TOPIC`. Hooks webhook: `hmac_secret_env` NOMBRA env vars arbitrarias por hook.

> **Estado de documentación:** repartido en 4 lugares, ninguno completo. `README §Configuration` (las comunes), `config.go` (doc-comments, completo para lo mapeado a `Config`), `DEPLOY.md` (subconjunto), `.env.example` (solo lo de compose). **Huérfanas** (no en `.env.example`, apenas en README): `RATE_LIMIT_*`, `DB_MAX_CONNS`, `APPXIMO_MAX_TX_OPS`, `APPXIMO_MAX_SSE_PER_TENANT`, `APPXIMO_SYNTHETIC`, `APPXIMO_PPROF_PORT`, `BACKUP_DIR`, `APPXIMO_SAFEGO_TIMEOUT`, `GOMEMLIMIT`, `GOGC`. No hay una referencia única autoritativa.
...
9. **[MENOR] Deploy del binario CUSTOM sin plantilla.** `examples/backend-guide` (main.go + schema.json) no trae Dockerfile/compose/deploy; el único Dockerfile es para `./cmd/appximo`. Un backend custom adapta todo a mano.
...
3. Usuario de sistema, `/etc/appximo/schema.json`, `/etc/appximo/engine.env` (los 3 required + tuning), `chmod 600`.
4. Unit `appximo.service` (+ `appximo-worker.service`) copiado de la doc; `systemctl enable --now`.
...
2. Build **local** canónico: `scripts/build-engine.sh /tmp/appximo-deploy <shortSHA> <sha>`.
...
- **`make fleet-init`** — lo más cercano a "provisión automatizada" que existe: scaffold de manifest + secretos generados + DBs bootstrapeadas. Modelo a mirar para un `appximo init-prod`/`setup` de single-app.
...
- `go build ./cmd/appximo` → **OK**. Binario canónico stripped = **60.6 MB**.
