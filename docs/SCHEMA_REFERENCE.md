...
- **PUT vs PATCH** (`CollectUpdate`): PUT requires every non-auto required field (absent optional fields are written NULL — full replacement); PATCH validates and writes only the fields present. Both reject `id`, unknown fields, and `auto` fields in the body (→ `422`). A missing/RBAC-excluded row → `404` (never `403`, to avoid leaking existence). A state-machine transition that is not allowed → `422` "invalid transition for ...".

#### List query parameters (`pkg/query/builder.go`)

- **Filters:** `?filter[field]=v` (implies `eq`) or `?filter[field][op]=v`. The bracket pattern is `^filter\[([a-z][a-z0-9_]*)\](?:\[([a-z]+)\])?$`. Valid ops per field type: `string`/`text` → `eq`, `partial` (`ILIKE %v%`), `start` (`ILIKE v%`); `int`/`int64`/`float64` → `eq`, `gt`, `gte`, `lt`, `lte`; `time` → those five plus `after`/`before`; `uuid`/`bool` → `eq` only. Unknown field → `400` "unknown filter field"; incompatible op → `400` listing the allowed ops. With curl, brackets need `-g`: `curl -g '...?filter[status][eq]=open'`.
...
- **Pagination — offset (discouraged):** `?page=` (1-based, default `1`, capped at `10000`) with `?per_page=` (default `20`, max `100`). A non-integer **or non-positive** `page`/`per_page` → `400` naming the value ("invalid page parameter \"0\": must be a positive integer"). Before 2026-08-01 a value ≤ 0 was silently served as page 1, even though that same message already said "must be a positive integer" (ADR-024). A value ABOVE the cap is still clamped, not rejected — the cap is documented and `meta` reports the effective value.
...
- **Limits:** body capped at 1 MiB (→ `413`); at most **50 root selections** per operation and **2000 total selections** across the document (alias-amplification guard), else the query is rejected with "query too complex". There is no separate depth counter.
