...
- **Sort:** `?sort=field&order=asc|desc` — ONE field only; default direction is `ASC`, `desc` (case-insensitive) flips it. Alternative `?order[field]=desc` also works and, when both are present, the `order[...]` form overrides `sort`. An unknown sort/order field is silently ignored. Multi-field or `sort=field:desc` is silently ignored.
...
- **Pagination — offset (discouraged):** `?page=` (1-based, default `1`, capped at `10000`) with `?per_page=` (default `20`, max `100`). A non-integer `page`/`per_page` → `400`; a value ≤ 0 is silently ignored (the default is kept), it is not an error.
