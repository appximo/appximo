...
- **The scheduled send policy (ADR-034, VOZ-DELTA-S1)** — two more optional keys in the same block:
  - `"notify": "changes"` (default) sends the morning digest ONLY when something changed since the last one — an attention count, its states, rows that arrived today, or the traffic light going up or landing on green; plain motion (`9 nuevos`) is not a change. `"always"` sends the daily report regardless. The manual `resumen` command always answers; silence is a property of the automatic send.
  - `"quiet_days": 7` (default; `0` = never) is the heartbeat: after that many consecutive silent scheduled runs, one short "sigo acá, todo igual" message goes out and the streak resets — a quiet channel stays distinguishable from a dead one. `estado` also prints the last scheduled evaluation.
  - Validated at load: `summary_notify_invalid` (only `changes`/`always`), `summary_quiet_days_negative`. A block with only policy keys (no `resources`) is legal.
  - Behind it: ONE snapshot per (tenant, role, day) in `public.summary_snapshots` — the digest compares against the most recent row from a previous day, writes today's, and prunes the rest (never a history). The `/api/summary` JSON carries `baseline`, `changed`, `change_reasons`; `?mode=scheduled` adds `should_send`/`send_reason`/`silent_streak` and records the decision.
...
- **What it is.** The words the owner actually says for this resource when its schema name is not the word they use — «pedidos» for `ordenes`, «mascotas» for an English `pets`. The voice channel (`POST /api/ask`, the Telegram bot, a Siri shortcut) recognizes an alias exactly like the schema name: singular/plural, accent-insensitive, for questions («cuántos pedidos hay hoy» → the deterministic parser, US$ 0) and for writes («cancela el pedido ORD-1003»). The model's vocabulary lists them too (`ordenes (also called: pedidos, ventas)`), so the rare question that still needs the model maps them from the schema, not from a guess. **The engine wires no domain word: if the schema does not declare it, the parser does not know it — and that is correct.** The reply always uses the schema's own word (`13 ordenes`), never the alias.
...
- **Voice.** The vocabulary marks the range (`[time range horario: inicio..fin, default duration 1h, no overlap]`); «qué tengo mañana» lists what is scheduled (overlaps), «tengo algo a las 4» what contains 16:00, «cuándo estoy libre el jueves» the gaps; «agenda reunión con Fabián mañana de 4 a 5» is settled by the deterministic parser (US$ 0) and the confirmation runs the conflict check: «⚠️ Ya tienes «reunión con Fabián» de 16:00 a 17:00. … ¿Igual lo agendo?» — a yes on an invertible rule (`eq` on a bool) writes the row as NOT blocking (`ocupa: no`) and says so; a rule that cannot be inverted asks for another time.
...
- A map from a DECLARED enum value to the words people say for it. The voice parser consumes an alias exactly like the value («qué pedidos están sin pagar» → `estado = pendiente_pago`; «marca como cobrada…» → a transition to `pagada`), including Spanish gender/number forms (`cobrada` also matches «cobrado», «cobrados»). The model's vocabulary shows them beside the value.
...
- **Conditions and `fields` are enforced on CREATE too** (mass-assignment block, `EnforceCreateRBAC`, `pkg/codegen/builder.go`, shared by REST POST and GraphQL `create…`):
  ...
  - the condition field is **forced** to the caller's resolved value; if the body supplies a *different* non-null value for it, the create is **rejected with 403** `field "<f>" must match the authenticated principal` (`pkg/codegen/rbac_write.go`). So an owner-scoped role can only create rows attributed to itself.
- **…and on UPDATE the identity column is server-owned too** (`EnforceUpdateRBAC`, same file, MOTOR-AUTORIZACION-S1 / ADR-027): for a condition bound to `$user_id` / `$external_client_id`, an update body supplying the column with anything but the caller's own id (another id, `null`) is the **same 403**, on REST PUT/PATCH, GraphQL `update…`, `POST /api/transaction` and `Ctx.Update`; judged on the client body before the row lookup (another principal's row stays 404) and before the field allowlist (explicit 403, never a silent drop). A PUT that omits a nullable condition column keeps the caller's value (never NULL); re-sending the caller's own id is a no-op. A LITERAL condition is not bound (a visibility filter, not ownership). Transferring a record is the job of an unscoped role or a custom handler — never the client body.
- **A state-machine field can never be null:** `PATCH {"status": null}` or a `PUT` omitting the state field → 422 `rule: "state"` naming it (every door; it used to be a 500).
...
- **Field selection:** `?fields=a,b,c` (MOTOR-FIELDS-S1) projects the SQL `SELECT` list itself — `SELECT "id", "a", "b", "c" FROM …` — so an unlisted column is never read (a TOASTed `json`/`jsonb`/`text` value is detoasted only when the SELECT names it; measured: a page of 20 rows with a ~50 KB document, 1.4 MB / 190 TOAST blocks / ~100 ms → 1.8 KB / 0 blocks / ~4 ms). `id` always included; request order kept; a repeated NAME is a set. Unknown name → `400` "unknown field in fields: X (available: …)"; a name outside the role's allowlist is DROPPED from the projection (the allowlist wins, as on every read — not the filter/sort `ErrForbiddenField`, which defends a value oracle; a 403 would break role-agnostic clients); `?fields=` / `a,,b` / repeated parameter → named `400`s. Validated against the tenant's deployed surface (`readSurface`). Also on get-by-id (`SELECT <cols> FROM t WHERE id=$1`), the relation subroute (fields of the TARGET, `SELECT r."id", r."x" …`), and the ROOT of an `?include=` read (the base subquery carries the requested fields plus the FK/order columns the wrapper's joins need — `includeBuilder.baseRefs` — never `SELECT *`, which the top-N sort would detoast; embeds are never projected). `?fields=` on `/aggregate` is the usual unknown-function 400. Absent ⇒ `SELECT *`, byte-identical.
