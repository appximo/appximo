...
func (h *Handlers) ListClients(w http.ResponseWriter, r *http.Request) {
	...
	query := "SELECT * FROM clients ORDER BY id LIMIT 100"
	var queryArgs []any
	if evalResult != nil && evalResult.Condition != nil {
		query = fmt.Sprintf("SELECT * FROM clients WHERE %s = $1 ORDER BY id LIMIT 100", evalResult.Condition.Field)
...
func (h *Handlers) CreateClients(w http.ResponseWriter, r *http.Request) {
	...
	hookRes, hookErr := h.hr.RunBeforeHook(r.Context(), h.hookConfig("clients", "before_create"), body, nil)
	if hookErr != nil {
	...
	query := fmt.Sprintf("INSERT INTO clients (%s) VALUES (%s) RETURNING *", cols, placeholders)
	result, err := h.tdb.ExecRowsTenant(r.Context(), tc.PGSchema, query, args...)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
	if afterHook := h.hookConfig("clients", "after_create"); afterHook != nil {
...
func (h *Handlers) GetClientsByID(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.tdb.QueryTenant(r.Context(), tc.PGSchema, "SELECT * FROM clients WHERE id = $1", idStr)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h *Handlers) DeleteClients(w http.ResponseWriter, r *http.Request) {
	...
	affected, err := h.tdb.ExecTenant(r.Context(), tc.PGSchema, "DELETE FROM clients WHERE id = $1", idStr)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
