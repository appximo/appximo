...
func (h *Handlers) GetIncidentsGuide(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.tdb.QueryTenant(r.Context(), tc.PGSchema,
		"SELECT r.* FROM guides r JOIN incidents src ON src.guide_id = r.id WHERE src.id = $1",
		idStr)
	if err != nil {
		writeDBError(w, err)
		return
...
func (h *Handlers) GetIncidentsReportedBy(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.tdb.QueryTenant(r.Context(), tc.PGSchema,
		"SELECT r.* FROM users r JOIN incidents src ON src.reported_by = r.id WHERE src.id = $1",
		idStr)
	if err != nil {
		writeDBError(w, err)
		return
