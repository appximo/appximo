...
func (h *Handlers) GetIncidentsByID(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.tdb.QueryTenant(r.Context(), tc.PGSchema, "SELECT * FROM incidents WHERE id = $1", id)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h *Handlers) DeleteIncidents(w http.ResponseWriter, r *http.Request) {
	tc := tenant.MustFromCtx(r.Context())
	id := chi.URLParam(r, "id")
	affected, err := h.tdb.ExecTenant(r.Context(), tc.PGSchema, "DELETE FROM incidents WHERE id = $1", id)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
